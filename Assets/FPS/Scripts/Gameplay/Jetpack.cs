    ...
    public class Jetpack : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (jetpackIsInUse)
            {
                ...
                float totalAcceleration = JetpackAcceleration * SkillSocket.Instance.jetpack;
                ...
                // consume fuel
                CurrentFillRatio = CurrentFillRatio - (Time.deltaTime / ConsumeDuration * 1/ SkillSocket.Instance.jetpack);
            ...
            else
            {
                ...
                if (IsJetpackUnlocked && Time.time - m_LastTimeOfUse >= RefillDelay * 1/ SkillSocket.Instance.jetpack)
                {
                    float refillRate = 1 / (m_PlayerCharacterController.IsGrounded
                        ? RefillDurationGrounded
                        : RefillDurationInTheAir);
                    CurrentFillRatio = CurrentFillRatio + Time.deltaTime * refillRate * SkillSocket.Instance.jetpack;
                }
